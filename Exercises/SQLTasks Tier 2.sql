...
SELECT concat(surname, ", ", firstname, ": ", name) as tennis_court_members
FROM (
	SELECT DISTINCT b.memid, b.facid, f.name
	FROM Bookings as b
		INNER JOIN Facilities as f
		ON b.facid = f.facid
		WHERE f.facid IN (0, 1)) as a
...
FROM (
	...
	FROM Bookings AS b
		...
		LEFT JOIN Facilities AS f
		ON b.facid = f.facid
	WHERE DATE(starttime) = '2012-09-14') AS source
WHERE cost > 30
ORDER BY cost DESC
...
FROM (
    ...
    GROUP BY name
    ORDER BY total_revenue) AS next
...
SELECT f.name, SUM(b.slots) AS member_bookings
...
SELECT f.name, month(starttime) as month, SUM(b.slots) AS member_bookings
FROM Facilities AS f
	LEFT JOIN Bookings AS b
	ON f.facid = b.facid
WHERE b.memid > 0
